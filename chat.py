...
def handle_set_name(name):
    ...
    # Send presence snapshot to the user who just joined
    users = [
        { 'sid': sid, 'name': client_name, 'ip': client_ip }
        for sid, (client_name, client_ip) in clients.items()
    ]
    emit('presence_snapshot', { 'users': users, 'system': [] }, room=request.sid)
    # Broadcast presence join to everyone else
    emit('presence_join', { 'sid': request.sid, 'name': name, 'ip': ip }, room='chatroom', include_self=False)
...
def handle_disconnect():
    ...
    if sid in clients:
        ...
        emit('presence_leave', { 'sid': sid, 'name': name, 'ip': ip_addr }, room='chatroom', include_self=False)
        del clients[sid]
